...
class YoutubeSongsAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    var youtubeSongs: List<Song> = emptyList()
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            TYPE_SONG_HEADER -> {
                (holder as HeaderViewHolder).bind(youtubeSongs.size)
            }
            TYPE_SONG_ITEM -> {
                val song = youtubeSongs[position + if (showHeader) -1 else 0]
    ...
    fun reorderSong(from: Int, to: Int) {
        youtubeSongs.moveElement(from, to)
    ...
    fun getSongForPosition(position: Int): Song? {
        return if (showHeader) {
            if (position == 0) {
                ...
                youtubeSongs[position - 1]
            }
        } else {
            youtubeSongs[position]
        }
    }
}
